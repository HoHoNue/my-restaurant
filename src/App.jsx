 ...
 * [필독] 사장님 설정 영역
 */
const ADMIN_PASSCODE = "1234"; // 관리자 모드 접속 코드
const apiKey = ""; // Gemini AI API Key (필요 시 입력)

// --- Firebase 설정 ---
// 본인의 Firebase 콘솔에서 복사한 실제 정보를 여기에 입력해야 데이터가 저장됩니다.
...
const appId = typeof __app_id !== 'undefined' ? __app_id : "store-production-final";
...
const App = () => {
  ...
  const [settings, setSettings] = useState({
    ...
    introImgScale: 100,
    introText: "가장 신선한 재료로 최고의 맛을 선사합니다.",
    naverReviewUrl: "", 
    categories: ["전체", "면류", "요리류"],
    openingHours: {
      sun: { open: "11:00", close: "21:00", active: true },
      mon: { open: "11:00", close: "21:00", active: true },
      tue: { open: "11:00", close: "21:00", active: false }, 
  ...
  useEffect(() => {
    ...
    }, (err) => console.error("Firestore 로드 실패:", err));
  ...
  if (loading) return <div className="h-screen flex items-center justify-center text-gray-400 font-bold tracking-widest uppercase">데이터 동기화 중...</div>;
      ...
      <div className={`${isAdminMode ? 'hidden' : 'block animate-in fade-in duration-1000'}`}>
        ...
        <header className="relative h-[80vh] md:h-[90vh] flex items-center justify-center overflow-hidden">
          ...
          <div className="relative z-10 text-center px-4 max-w-4xl animate-in slide-in-from-top-12 duration-1000 text-white">
            <div className="inline-block px-4 py-1.5 bg-white/10 backdrop-blur-md rounded-full border border-white/20 text-white text-[10px] font-black tracking-widest uppercase mb-6 tracking-widest">Premium Selection</div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-20 space-y-32">
          ...
          <section className="animate-in slide-in-from-bottom-12 duration-700">
            <div className="flex items-center gap-2 mb-8 justify-center sm:justify-start">
              ...
              <h2 className="text-2xl font-black tracking-tighter uppercase">영업 시간 안내</h2>
            </div>
          ...
          <section id="menu-list" className="scroll-mt-24">
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-12">
              <div className="text-center lg:text-left">
                ...
                <p className="text-gray-400 font-medium break-keep text-sm">더블 클릭으로 메뉴를 찜하고 좌우로 밀어서 확인하세요.</p>
              </div>
              ...
              <div ref={menuScrollRef} className="flex overflow-x-auto gap-4 md:gap-8 pb-12 px-4 -mx-4 snap-x snap-mandatory no-scrollbar cursor-grab active:cursor-grabbing">
                {filteredMenu.map((item) => (
                  <div key={item.id} onDoubleClick={() => toggleLike(item.id)} className={`flex-none w-[280px] sm:w-[320px] md:w-[340px] lg:w-[280px] snap-center relative flex flex-col p-6 rounded-[3rem] bg-white border border-gray-100 shadow-sm transition-all duration-500 select-none hover:shadow-2xl hover:-translate-y-2 ${item.isRecommended ? 'ring-2 ring-orange-200 border-orange-200' : ''}`}>
                    ...
                    <div className="w-full aspect-square rounded-[2.5rem] overflow-hidden mb-6 relative shadow-md bg-gray-50 shrink-0">
                    ...
                    <div className="flex flex-col flex-1">
                      ...
                      <div className="min-h-[3.5rem]"><p className="text-gray-400 text-xs line-clamp-2 leading-relaxed break-keep font-medium">{item.desc || "메뉴에 대한 설명이 준비되어 있지 않습니다."}</p></div>
          ...
          <section className="relative overflow-hidden py-10">
            <div className="text-center mb-16 px-4"><h2 className="text-4xl md:text-5xl font-black tracking-tighter mb-4 break-keep uppercase">손님 리뷰</h2></div>
          ...
          {/* 오시는 길 */}
          <section id="map-area" className="pb-32 scroll-mt-24">
            <div className="flex flex-col sm:flex-row items-center justify-between mb-12 gap-6 text-center sm:text-left">
              <div><h2 className="text-4xl font-black tracking-tighter flex items-center gap-3 justify-center sm:justify-start break-keep uppercase">오시는 길</h2><p className="text-gray-400 mt-2 font-medium">가게 주소 정보를 입력해 주세요.</p></div>
              {settings.naverReviewUrl && <a href={settings.naverReviewUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 px-8 py-4 bg-[#2DB400] text-white rounded-full text-sm font-black shadow-2xl hover:brightness-110 transition-all active:scale-95"><MessageSquare size={16} /> 네이버 리뷰 바로가기 <ExternalLink size={14} /></a>}
            </div>
            <div className="aspect-video bg-gray-100 rounded-[4rem] border-[12px] border-white shadow-2xl flex items-center justify-center text-gray-200 font-black text-2xl relative"><MapPin size={64} className="text-orange-500 animate-bounce" /><span className="tracking-tighter uppercase italic ml-4">지도 데이터 준비 중</span></div>
          </section>
        </div>
        ...
        <footer className="bg-gray-900 py-20 px-4 text-center">
            ...
            <p className="text-gray-500 text-sm font-bold tracking-widest">© 2024 {settings.name}. All Rights Reserved.</p>
        </footer>
      </div>
          ...
          <div className="max-w-4xl mx-auto px-6 py-12">
            ...
            <header className="sticky top-0 z-50 flex flex-col bg-white p-6 rounded-[2.5rem] shadow-xl border border-gray-100 mb-12">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4"><div className="w-14 h-14 bg-indigo-600 rounded-[1.5rem] flex items-center justify-center text-white shadow-2xl"><Settings size={28} /></div><div><h2 className="text-2xl font-black text-gray-900">관리자 대시보드</h2><p className="text-[10px] text-gray-400 font-black uppercase">Store Manager</p></div></div>
              ...
              <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-2 px-1">
                <button onClick={() => scrollToAdminSection(adminCategoryRef)} className="flex items-center gap-2 px-6 py-3 bg-indigo-50 text-indigo-600 rounded-2xl text-xs font-black hover:bg-indigo-600 hover:text-white whitespace-nowrap shadow-sm"><LayoutGrid size={14}/> 카테고리</button>
                <button onClick={() => scrollToAdminSection(adminMenuRef)} className="flex items-center gap-2 px-6 py-3 bg-orange-50 text-orange-600 rounded-2xl text-xs font-black hover:bg-orange-600 hover:text-white whitespace-nowrap shadow-sm"><MenuIcon size={14}/> 메뉴 관리</button>
                <button onClick={() => scrollToAdminSection(adminReviewRef)} className="flex items-center gap-2 px-6 py-3 bg-green-50 text-green-600 rounded-2xl text-xs font-black hover:bg-green-600 hover:text-white whitespace-nowrap shadow-sm"><MessageSquare size={14}/> 리뷰 관리</button>
                <button onClick={() => scrollToAdminSection(adminBrandingRef)} className="flex items-center gap-2 px-6 py-3 bg-gray-50 text-gray-600 rounded-2xl text-xs font-black hover:bg-gray-900 hover:text-white whitespace-nowrap shadow-sm"><Info size={14}/> 정보 설정</button>
              </div>
            </header>
            ...
            <div className="space-y-24 pb-40">
              ...
              <section ref={adminCategoryRef} className="space-y-8 scroll-mt-60">
                <div className="flex items-center gap-3"><div className="w-2 h-8 bg-indigo-500 rounded-full shadow-lg"></div><h3 className="text-xl font-black tracking-tighter">메뉴 카테고리 구성</h3></div>
                <div className="p-10 bg-white rounded-[3rem] shadow-sm border border-gray-100 space-y-6">
                  <div className="flex gap-3"><input type="text" id="new-cat-input-v8" placeholder="새 카테고리 입력" className="flex-1 p-5 rounded-2xl bg-gray-50 outline-none font-bold" /><button onClick={() => { const input = document.getElementById('new-cat-input-v8'); if(input.value) { setSettings({...settings, categories: [...settings.categories, input.value]}); input.value = ""; } }} className="px-8 bg-indigo-600 text-white rounded-2xl font-black shadow-lg">추가</button></div>
              ...
              <section ref={adminMenuRef} className="space-y-8 scroll-mt-60">
                <div className="flex items-center gap-3"><div className="w-2 h-8 bg-orange-500 rounded-full shadow-lg"></div><h3 className="text-xl font-black tracking-tighter">전체 메뉴 리스트</h3></div>
                <div className="space-y-6">
                  {settings.menuItems.map((item, idx) => (
                    <div key={item.id} className={`p-8 bg-white rounded-[3rem] border-2 transition-all ${item.isRecommended ? 'border-orange-500/30 ring-8 ring-orange-50' : 'border-gray-50'}`}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4"><input value={item.name} onChange={(e) => { const ni = [...settings.menuItems]; ni[idx].name = e.target.value; setSettings({...settings, menuItems: ni}); }} placeholder="메뉴명" className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none font-bold" /><input value={item.price} onChange={(e) => { const ni = [...settings.menuItems]; ni[idx].price = e.target.value; setSettings({...settings, menuItems: ni}); }} placeholder="가격" className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none font-bold" /><select value={item.category} onChange={(e) => { const ni = [...settings.menuItems]; ni[idx].category = e.target.value; setSettings({...settings, menuItems: ni}); }} className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none font-bold appearance-none shadow-inner">{settings.categories.filter(c => c !== "전체").map(c => <option key={c} value={c}>{c}</option>)}</select></div>
              ...
              <section ref={adminReviewRef} className="space-y-8 scroll-mt-60">
                <div className="flex items-center gap-3"><div className="w-2 h-8 bg-green-500 rounded-full shadow-lg"></div><h3 className="text-xl font-black tracking-tighter">리뷰 관리</h3></div>
              ...
              <section ref={adminBrandingRef} className="space-y-8 scroll-mt-60">
                <div className="flex items-center gap-3"><div className="w-2 h-8 bg-gray-900 rounded-full"></div><h3 className="text-xl font-black tracking-tighter">정보 설정</h3></div>
                <div className="p-10 bg-white rounded-[4rem] shadow-sm border border-gray-100 space-y-10">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-2"><label className="text-xs font-black text-gray-400 ml-1">상호명</label><input value={settings.name} onChange={(e) => setSettings({...settings, name: e.target.value})} className="w-full p-5 bg-gray-50 border-none rounded-2xl font-black text-2xl outline-none shadow-inner" /></div><div className="space-y-2"><label className="text-xs font-black text-gray-400 ml-1">리뷰 주소</label><input value={settings.naverReviewUrl} onChange={(e) => setSettings({...settings, naverReviewUrl: e.target.value})} placeholder="네이버 플레이스 URL" className="w-full p-5 bg-gray-50 border-none rounded-2xl text-sm font-bold shadow-inner" /></div></div>
                  <div className="space-y-4"><div className="flex justify-between items-center"><label className="text-xs font-black text-gray-400">대문 이미지</label><button onClick={handleGenerateIntro} className="text-indigo-600 flex items-center gap-2 font-black text-xs hover:underline"><Sparkles size={14}/> AI 자동 생성</button></div><div className="flex flex-col sm:flex-row gap-8 items-center"><div className="w-full sm:w-64 aspect-video rounded-[2rem] overflow-hidden border-4 border-white shadow-2xl shrink-0"><img src={settings.introImg} className="w-full h-full object-cover" /></div><label className="w-full h-32 flex flex-col items-center justify-center border-4 border-dashed border-gray-50 rounded-[2rem] cursor-pointer hover:bg-gray-50 transition-all text-gray-300"><Upload size={32} /><span className="text-[10px] mt-2 font-black uppercase">사진 업로드</span><input type="file" className="hidden" onChange={(e) => handleImageUpload(e, (url) => setSettings({...settings, introImg: url}))} /></label></div></div>
                  <div className="space-y-2"><label className="text-xs font-black text-gray-400 ml-1">소개글</label><textarea value={settings.introText} onChange={(e) => setSettings({...settings, introText: e.target.value})} className="w-full p-6 bg-gray-50 border-none rounded-[2rem] text-base font-medium min-h-[120px] shadow-inner" /></div>
                </div>
              </section>
